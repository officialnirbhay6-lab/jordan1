...
async function crawlWebsiteForEmails(browser, url) {
  ...
  // SKIP CRAWLING HEAVY SOCIAL MEDIA & BUSINESS DIRECTORIES
  // Visiting directories is extremely slow and almost never yields direct business emails.
  const lowerUrl = url.toLowerCase();
  const skippedDomains = [
    'facebook.com', 'instagram.com', 'zomato.com', 'justdial.com', 'youtube.com', 
    'twitter.com', 'linkedin.com', 'tripadvisor.com', 'tripadvisor.in', 'pinterest.com', 
    'google.com', 'maps.google.com', 'google.co.in', 'yelp.com', 'indiamart.com', 
    'facebook.co', 'justdial.co', 'justdial.in'
  ];

  const isDirectoryOrSocial = skippedDomains.some(domain => lowerUrl.includes(domain));
  if (isDirectoryOrSocial) {
    log(`Skipping direct crawl for social media or directory listing: ${url}`);
    return null;
  }
  ...
  try {
    ...
    // De-duplicate emails and filter out trash domains or placeholders
    emails = [...new Set(emails)].filter(email => {
      ...
      // Filter out trash/placeholder/developer domains
      const trashDomains = [
        'sentry.io', 'wixpress.com', 'wix.com', 'mysite.com', 'example.com', 
        'domain.com', 'shopify.com', 'wordpress.org', 'wordpress.com', 'sentry-next.wixpress.com',
        'email.com', 'yourname'
      ];
      const isTrash = trashDomains.some(domain => lower.includes(domain));
      if (isTrash) return false;

      // Filter out obviously fake emails (e.g. contains wixpress, sentry, or typical templates)
      if (lower.includes('sentry') || lower.includes('wixpress') || lower.includes('example') || lower.includes('yourname')) {
        return false;
      }
    ...
    for (const subpageUrl of uniqueSubpages) {
      try {
        ...
        subEmails = [...new Set(subEmails)].filter(e => {
          ...
          if (l.endsWith('.png') || l.endsWith('.jpg') || l.endsWith('.jpeg') || l.endsWith('.gif')) {
            return false;
          }
          const trashDomains = [
            'sentry.io', 'wixpress.com', 'wix.com', 'mysite.com', 'example.com', 
          ...
          const isTrash = trashDomains.some(domain => l.includes(domain));
          if (isTrash) return false;

          if (l.includes('sentry') || l.includes('wixpress') || l.includes('example') || l.includes('yourname')) {
            return false;
          }
          return true;
        });
...
export async function scrapeGoogleMapsLeads(keyword, location, limit = 15) {
  ...
  try {
    ...
    // SINGLE PLACE DIRECT REDIRECT DETECTION
    if (currentUrl.includes('/maps/place/')) {
      log('Search query redirected directly to a single business detail view!');
      uniqueLinks = [currentUrl];
    } else {
        ...
        try {
          await page.evaluate(() => {
            const feed = document.querySelector('div[role="feed"]') || 
                         document.querySelector('.m67qMC') || 
                         document.querySelector('[class*="scrollable"]');
            if (feed) {
              feed.scrollTop = feed.scrollHeight;
            } else {
              window.scrollBy(0, 1000); // Fallback to window scroll
            }
          });
          await delay(1500);
          scrollCount++;
      ...
      uniqueLinks = [...new Set(businessLinks)].slice(0, limit);
    ...
    // SWEET SPOT CONCURRENCY = 2
    // Doubles the scraping speed while maintaining fully secure memory bounds under 200MB.
    const concurrency = 2; 
    const leadsFile = path.join(process.cwd(), 'leads.json');
    ...
    for (let i = 0; i < uniqueLinks.length; i += concurrency) {
      ...
      const batchPromises = batch.map(async (link, index) => {
        ...
        try {
          ...
          await workerPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36');
          await workerPage.setViewport({ width: 1200, height: 900 });
          await workerPage.setDefaultNavigationTimeout(12000); // 12s snappy timeout for GMB detail loading

          // Go to listing detail view
          await workerPage.goto(link, { waitUntil: 'domcontentloaded' });
          await delay(800); // Snappy delay to let detail items render

          // Extract GMB details with robust selector fallbacks
          const details = await workerPage.evaluate(() => {
            ...
            const name = getElementText('h1') || 
                         getElementText('.DUw3O') || 
                         getElementText('[class*="header-title"]') || 
                         'Unknown Business';
            ...
            const category = getElementText('button[class*="D72N1"]') || 
                             getElementText('.fontBodyMedium span') || 
                             '';
          ...
          if (isDuplicate) {
            log(`[Worker] Skipping duplicate lead from database: "${details.name}"`);
            return;
          }
          ...
          log(`Failed scraping GMB listing [${link}]: ${leadErr.message}`);
        } finally {
          if (workerPage) {
