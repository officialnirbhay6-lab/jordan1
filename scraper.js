...
async function crawlWebsiteForEmails(browser, url) {
  ...
  try {
    ...
    await page.setDefaultNavigationTimeout(10000); // 10s snappy limit for homepage loading
    ...
    for (const subpageUrl of uniqueSubpages) {
      try {
        ...
        await page.setDefaultNavigationTimeout(6000); // 6s tight timeout for contact pages
...
export async function scrapeGoogleMapsLeads(keyword, location, limit = 15) {
  ...
  try {
    ...
    // If the query leads directly to a single business, Google Maps redirects directly to its detail page.
    if (currentUrl.includes('/maps/place/')) {
      ...
      while (scrollCount < maxScrolls) {
        try {
          await page.evaluate(() => {
            // Google Maps listing panel scroll selectors
      ...
      const businessLinks = await page.evaluate(() => {
        // Find all anchors referencing a maps place card
    ...
    // Close the primary search page as we don't need it anymore (saving RAM immediately!)
    await page.close();

    // SUPER LOW-MEMORY STRATEGY: Process sequential listings one-by-one (concurrency = 1)
    // This absolutely guarantees that only a single browser tab is running at any given time.
    const concurrency = 1; 
    const leadsFile = path.join(process.cwd(), 'leads.json');
    ...
    for (let i = 0; i < uniqueLinks.length; i += concurrency) {
      ...
      const batchPromises = batch.map(async (link, index) => {
        ...
        log(`Scraping lead ${globalIndex + 1}/${uniqueLinks.length} (Sequential Worker): ${link}`);
        ...
        try {
          ...
          await delay(1800); // Wait for detail panel values to settle
          ...
          const details = await workerPage.evaluate(() => {
            ...
            // Rating and reviews Selector Fallbacks
            ...
            // Business Category Selector Fallbacks
          ...
          // Qualify lead - Target GMB listings lacking website or unoptimized
