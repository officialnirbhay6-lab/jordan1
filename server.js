...
registerMessageCallback(async (msg) => {
  ...
  // Ignore status broadcasts, media captions, or automatic digests to prevent recursive loops (large 2000 limit allows long queries)
  if (!msg.body || msg.body.length > 2000 || msg.from.includes('broadcast')) {
    return;
  }

  // Prevent infinite loops: ignore any message sent by the bot itself containing its own signature phrases!
  if (msg.fromMe === true && (
    msg.body.includes('Acknowledged') || 
    msg.body.includes('Outreach Digest') || 
    msg.body.includes('service') ||
    msg.body.includes('90skids.digital')
  )) {
    return;
  }
  ...
  // Clean summon name "jordan" if present, but DO NOT require it.
  // This lets Nirbhay Sir trigger Jordan with any text, query, or question immediately!
  let cleanText = text.replace(/jordan/g, '').trim().replace(/^[\s,.\?!;:]+|[\s,.\?!;:]+$/g, '');
  if (cleanText === '') {
    cleanText = 'hello'; // Default to greeting if empty
  }

  // Advanced query parser supporting "find 30 business without website in delhi", "scrape 15 hotels in patna", etc.
  let query = cleanText.toLowerCase();

  // Extract custom target count if specified (e.g. 30, 15, 50)
  const countRegex = /(\d+)/;
  const countMatch = query.match(countRegex);
  let customLimit = 20; // Default limit if not specified
  if (countMatch) {
    customLimit = parseInt(countMatch[1]) || 20;
    query = query.replace(countRegex, '').trim();
  }

  // Clean filler words to isolate the core search intent
  query = query
    .replace(/(?:without|no)\s+websites?/g, '')
    .replace(/(?:cold\s+)?emails?\s+(?:them|outreach)?/g, '')
    .replace(/\s+and\s+/g, ' ')
    .replace(/\b(prospects|businesses|listings|places|shops)\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();

  // Pattern 1: [niche] (leads) from/in/at [location]
  const regex1 = /([a-zA-Z\s\-_]+)\s+(?:leads?\s+)?(?:from|in|at)\s+([a-zA-Z\s\-_]+)/i;
  const match = query.match(regex1);

  let customKeyword = '';
  let customLocation = '';

  if (match) {
    customKeyword = match[1].trim();
    customLocation = match[2].trim();
  } else {
    // Pattern 2: [niche] [location] fallback (e.g. "clinic delhi")
    const words = query.split(/\s+/);
    if (words.length >= 2) {
      customLocation = words[words.length - 1].trim();
      customKeyword = words.slice(0, words.length - 1).join(' ').trim();
    }
  }

  // Final clean-up of action keywords
  if (customKeyword) {
    customKeyword = customKeyword.replace(/\b(find|search|get|scrape|give)\b/gi, '').trim();
  }

  // Check if we successfully parsed a dynamic niche & location search
  if (customKeyword && customLocation && !['status', 'email', 'mail', 'outreach', 'help', 'hi', 'hello', 'hlw', 'hey'].includes(customKeyword)) {
    log(`Interactive WhatsApp command: Nirbhay requested ${customLimit} dynamic leads for "${customKeyword}" in "${customLocation}"!`);

    // Reply immediately using Jordan's persona to acknowledge the message fast
    await msg.reply(`Acknowledged, Nirbhay Sir! 🚀 Your loyal agent Jordan is in your service, Sir!\n\nI have parsed your custom request:\n🎯 *Target Niche*: ${customKeyword}\n📍 *Target Location*: ${customLocation}\n📊 *Target Count*: ${customLimit} listings\n\nStarting my Puppeteer engine to search Google Maps and extract contacts (prioritizing phone numbers) immediately. I will send you the lead digest and auto-email proposals as soon as it's completed! (Typically takes 2-4 minutes).`);

    // Run custom scraping and outreach in background
    (async () => {
      try {
        const leads = await scrapeGoogleMapsLeads(customKeyword, customLocation, customLimit);
        ...
        if (leads.length === 0) {
          try {
            ...
            log(`Failed to send 0 leads message: ${wsErr.message}`);
          }
          return;
        }
        ...
        for (const lead of leads) {
          if (lead.email) {
            try {
              ...
              if (sent) {
                emailsSent++;
                // Add 3-second spacing delay to prevent Gmail/SMTP connection blocks
        ...
        // Auto direct WhatsApp B2B pitch to leads lacking website
        ...
        for (const lead of leads) {
          if (lead.isHighPotential && !lead.website && lead.normalizedPhone) {
            try {
              ...
              if (sent) {
                whatsappPitchesSent++;
                // Add 2-second safety delay to protect WhatsApp account metrics
                await new Promise(res => setTimeout(res, 2000));
              }
            } catch (err) {
              log(`Failed custom WhatsApp B2B pitch outreach to ${lead.name}: ${err.message}`);
            }
          }
        }
        ...
        const digestMsg = buildWhatsAppDigest(customLocation, leads, highPotential, emailsSent, whatsappPitchesSent);
        try {
          await sendWhatsAppMessage(userPhone, digestMsg);
        } catch (wsErr) {
          log(`Failed to send custom lead digest WhatsApp message: ${wsErr.message}`);
        }
        ...
        try {
          ...
          log(`Failed to send scrape error WhatsApp message: ${wsErr.message}`);
        }
      }
    })();
    ...
    const statusMsg = `🤖 *Jordan - 90skids.digital Agent Status* 🤖\n\n` +
      ...
      `• *give leads now* - Scrapes current target immediately.\n` +
      `• *send emails now* - Sends emails to pending leads.\n` +
      `• *status* - Shows this status message.`;
    
    await msg.reply(statusMsg);

  } else if (cleanText === 'help' || cleanText === 'hi' || cleanText === 'hello' || cleanText === 'hlw' || cleanText === 'hey') {
    const welcomeMsg = `Your agent Jordan is in your service, Nirbhay Sir! 🤵‍♂️👋\n\n` +
      ...
      `*Simply reply with 'give leads now', 'send emails now', or 'status' and I will start immediately, Nirbhay Sir!*`;
