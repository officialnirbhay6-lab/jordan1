...
registerMessageCallback(async (msg) => {
  ...
  // Stricter check: only respond if the message is from Nirbhay Kumar's number
  if (msg.from !== userChatId) {
    log(`Message ignored (not from Nirbhay's configured number: ${userChatId})`);
    return;
  }
