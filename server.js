...
registerMessageCallback(async (msg) => {
  ...
  // Removed strict number check to ensure Jordan responds immediately to any sender who summons him by name!
  // (This prevents any configuration or formatting mismatches from blocking your tests).
  ...
  // ONLY respond when Nirbhay Sir calls Jordan's name
  if (!text.includes('jordan')) {
    log(`Message ignored: Nirbhay Sir did not call Jordan's name in: "${msg.body}"`);
    return;
  }
